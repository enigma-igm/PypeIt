        ...
        * self.all_dar
        ...
        for ff, fil in enumerate(self.spec2d):
            ...
            if not self.combine and not self.align:
                ...
                bins = self.spectrograph.get_datacube_bins(slitlength, minmax, numwav)
                ...
                if self.method in ['subpixel', 'ngp']:
                    ...
                    final_cube.to_file(os.path.join(self.output_dir, outfile), primary_hdr=self.all_header[ff], hdr=hdr,
                ...
                if image_phase:
                    for ff in range(self.numfiles):
                        # Calculate the shift
                            ...
                            log.info("Spatial shift of cube #{0:d}:\n".format(ff + 1) +
    ...
    def run(self):
        ...
        * Generate individual datacubes (combine=False) or one master datacube containing all exposures (combine=True).
        ...
        if self.combine and not self.align:
            log.warn("Combining frames without aligning them.\n" +
                    "Make sure that you know what you are doing!\n" +
                    "Even if your frames are taken at the same position,\n" +
        ...
        if self.method in ['subpixel', 'ngp']:
            ...
            if self.combine: #& self.align:
                ...
                log.info("Saving combined datacube as: {0:s}".format(str(combined_outfile)))
